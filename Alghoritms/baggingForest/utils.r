...
positions <- function(row,colname="POLYLINE") as.data.frame(do.call(rbind, fromJSON(as.vector(row[,colname]))))
...
timeToDay <- function(time) as.POSIXlt(time, origin="1970-01-01", tz="WET")
...
square_transform <- function(trip, edge_lat=0.00225, edge_long=0.003){
