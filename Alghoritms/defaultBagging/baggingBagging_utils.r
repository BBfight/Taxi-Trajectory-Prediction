### WARNING ###
###################################################################################
### THE RELATED UTILS GENERATES NOMINAL ATTRIUTES FOR CALLTYPE,DAYPHASE,WEKKDAY ###
###################################################################################
...
handlePredictions <- function(predictions){
  avg_lat <- mean(predictions[,1])
  avg_long <- mean(predictions[,2])
  
  temp <- predictions
  temp[,1] <- abs(temp[,1] - avg_lat)
  temp[,2] <- abs(temp[,2] - avg_long)
  
  min_lat <- predictions(which.min(temp[,1]))
  min_long <- predictions(which.min(temp[,2]))
  
  final <- c(min_lat,min_long)
